...
#define dB(Z,Zref) (20*log(Z/Zref)) 
...
float Zref;
uint8_t Alarm_counter = MUESTRAS_ALARMA; // cantidad de muestras que tomo debajo del umbral antes de activar la alarma
...
void loop() {
  ...
  switch (Chidori.estado) {
    ...
    case MIDIENDO:
      if (dB(Chidori.Z,Chidori.Ref) < UMBRAL) Alarm_counter --;
      if (Alarm_counter == 0) {
        Counter = T_ENVIO_DATOS; //Devuelvo el counter al valor original asi tengo mediciones limpias despues del frenado
...
void Medir_Impedancia() { // Mide la señal recibida y calcula la impedancia
  ...
  if(Counter > 0) Counter --;
      ...
      if(First_Measure){ // La primer medicion que envio a la pagina la tomo como referencia de la señal
    ...
    // Chequeo si la computadora esta conectada a la misma red de la GUI
      ...
      // Imprimo tambien los datos por monitor serie
      Serial.println(Chidori.Z);
