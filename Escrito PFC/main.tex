...
\end{equation}
Para el cálculo de las amplitudes ideales se adopta una señal de referencia de 14~mV pico a pico
a la salida del amplificador de instrumentación. Con esta entrada, la segunda etapa entrega
idealmente 140~mV pico a pico. Las mediciones del prototipo se presentan por separado en resultados.
...
300~$\Omega$ conduce a la entrada inversora del tercer operacional. Esta última resistencia carga
...
\begin{equation}
    ...
    =\frac{1000\times300}{1000+300}~\Omega\approx230{,}8~\Omega,
\end{equation}
\begin{equation}
    f_{\mathrm{HP1}}=\frac{1}{2\pi R_pC_a}
    \approx34{,}5~\mathrm{kHz}.
    \label{eq:fc1}
\end{equation}
Con el capacitor de 20~nF, la frecuencia de corte es aproximadamente 34,5~kHz al considerar la carga de
...
\begin{equation}
    ...
    {(1+sR_pC_a)(1+sR_fC_f)}.
\end{equation}
A 50~kHz su magnitud es aproximadamente 1,10: la amplificación de la tercera etapa supera levemente la
atenuación de las redes de filtrado a esa frecuencia. Los cortes de 34,5 y 67,7~kHz pertenecen a las
redes individuales; los límites de esta sección completa a $-3$~dB respecto de su máximo son
aproximadamente 19,2 y 121,4~kHz, con máximo a 48,3~kHz.
...
Permite obtener una señal del orden de 1,95~V pico a pico antes del detector a partir del
punto de funcionamiento adoptado de 14~mV pico a pico en la salida del amplificador de instrumentación.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        ...
        Primera & Seguidor & 1 \\
        Segunda & Inversor & 10 \\
        Tercera & Acoplamiento cargado y filtro inversor & 1,10 \\
...
\begin{equation}
    |H_{\mathrm{ac}}(50~\mathrm{kHz})|\approx139{,}0.
\end{equation}
Si se incluye un INA ideal con ganancia 5, el producto es aproximadamente 694,9. Para dimensionar
la salida a partir de los 14~mV pico a pico adoptados como referencia a la salida del amplificador
...
amplificador, antes del detector. La ganancia a 50~kHz es aproximadamente 139,0~V/V
(42,9~dB). Los límites de banda a $-3$~dB respecto del máximo son aproximadamente
25,3 y 91,5~kHz, con un ancho de banda de 66,3~kHz. La frecuencia de excitación de
...
La Tabla~\ref{tab:amplitudes_propuestas} presenta las amplitudes calculadas usando 14~mV pico a pico
en la salida del amplificador de instrumentación y las ganancias ideales de cada etapa.
\begin{table}[htbp]
    \centering
    \caption{Amplitudes calculadas a 50~kHz para una entrada de 14~mV pico a pico desde el amplificador de instrumentación.}
    \label{tab:amplitudes_propuestas}
    \begin{tabular}{lr}
        \hline
        Punto & Tensión pico a pico (mV) \\
        \hline
        Salida del amplificador de instrumentación & 14 \\
        Salida del primer seguidor & 14 \\
        Salida del amplificador de ganancia diez & 140 \\
        Salida del primer filtro inversor & 155 \\
        Salida del pasaaltos activo & 521 \\
        Salida del segundo seguidor & 428 \\
        Salida del amplificador final, antes del detector & 1946 \\
        \hline
    \end{tabular}
\end{table}
La salida final prevista de 1,95~V pico a pico corresponde a una amplitud de aproximadamente
0,97~V de pico para una senoide. En la sección de resultados se comparará esta estimación con la señal medida en el prototipo.
...
Al comparar con las amplitudes calculadas para la misma entrada de 14~mV pico a pico, la señal medida
antes del detector (1450~mV) es un 25\,\% menor que la prevista (1946~mV). La ganancia medida desde la
salida del INA hasta ese punto es aproximadamente 104, frente a 139 en el cálculo ideal. La
Tabla~\ref{tab:ganancias_comparadas} descompone la diferencia por etapa a partir de los valores de la
Tabla~\ref{tab:amplitudes_medidas}.

\begin{table}[htbp]
    \centering
    \caption{Ganancia de cada etapa a 50~kHz: valor ideal y valor obtenido de las mediciones.}
    \label{tab:ganancias_comparadas}
    \begin{tabular}{lrr}
        \hline
        Etapa & Ideal & Medida \\
        \hline
        Inversor de ganancia diez & 10 & 11,3 \\
        Primer filtro inversor & 1,10 & 1,01 \\
        Pasaaltos activo & 3,37 & 3,19 \\
        Pasabajos y seguidor & 0,821 & 0,647 \\
        Amplificador final & 4,55 & 4,39 \\
        Total desde la salida del INA & 139 & 104 \\
        \hline
    \end{tabular}
\end{table}

La mayor parte de la diferencia se concentra en el pasabajos, seguido del primer filtro inversor, cuya ganancia medida es un 8\,\% menor que la ideal. Una ganancia de 0,647 a 50~kHz equivale a
un polo cercano a 42~kHz y no a los 71,9~kHz nominales, lo que corresponde a una capacidad efectiva
del orden de 450~pF en lugar de 270~pF. Una posible causa es la capacidad parásita de la placa o de la
punta del osciloscopio, o la tolerancia del capacitor; esta hipótesis no se verificó. El pasaaltos activo y el
amplificador final quedan entre un 4 y un 6\,\% por debajo del valor ideal, compatible con las tolerancias de
los componentes y el ancho de banda finito de los operacionales. En la etapa de ganancia diez, la ganancia
medida supera en un 13\,\% a la ideal; como la entrada de 14~mV pico a pico es una señal pequeña para
el osciloscopio, es probable que ese valor tenga un error de lectura relativo apreciable.
